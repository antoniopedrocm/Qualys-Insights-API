...
function apiUrl(endpoint) {
  ...
  return new URL(cleanEndpoint, document.baseURI || window.location.href).toString();
}

async function apiCall(endpoint, needsAuth = true) {
  const headers = {
    'Content-Type': 'application/json',
    'X-Requested-With': 'QualysInsights'
  };
...
async function analyzeEffectiveness() {
  ...
  try {
    showLoading(true);
    const response = await fetch(apiUrl('/api/effectiveness'), {
      method: 'POST',
      headers: {
        'Content-Type': 'application/json',
        'X-Requested-With': 'QualysInsights'
...
async function exportExcel() {
  try {
    const response = await fetch(apiUrl('/api/export/vulnerabilities/excel'), {
      credentials: 'same-origin'
    });
...
async function exportCSV() {
  try {
    const response = await fetch(apiUrl('/api/export/vulnerabilities/csv'), {
      credentials: 'same-origin'
    });
